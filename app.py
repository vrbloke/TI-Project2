...
def reports():
	...
	habits = sql_select_c(conn,f'SELECT name, startDate, frequency, id FROM habit WHERE userID = {fl.session["userID"]}')
	today = date.today()
	iterable = []
	for habit in habits:
		...
		for instance in last10Instances:
			...
			res = sql_select_c(conn,f"SELECT * FROM activity WHERE habitID = {habit[3]} AND date = '{date.isoformat(instance)}'")
			printerr(res)
...
def uploadLocalHabit():
	conn = create_connection(dbpath)
	printerr(f'name: {fl.request.form["name"]}, startDate: {fl.request.form["startDate"]}, interval: {fl.request.form["interval"]}')
	if not isLoggedIn():
		return ('',204)
	# check if same-named habit exists
	same_name = sql_select_c(conn,f"""select name from habit where userID = {fl.session["userID"]} and name = '{fl.request.form["name"]}'""")
	printerr(same_name)
...
def uploadLocalActivity():
	...
	# find habit ID
	habitID = sql_select(f"""select id from habit where userID = {fl.session["userID"]} and name = '{fl.request.form["name"]}'""")[0][0]
